...
def read_image(img_path):
    ...
    while not got_img:
        try:
            img_type = 'RGB'
            img = Image.open(img_path).convert(img_type)#转换为RGB模式
            got_img = True
        except IOError:
...
class BaseDataset:
    def __init__(self, root='/home/zbc/data/reid',
        ...
        self._check_before_run()#检查上4个dir是否存在

        train = self._process_dir(self.train_dir, relabel=True)#获取图片的label和摄像机ID，和图片路径组合
        query = self._process_dir(self.query_dir, relabel=False)
        gallery = self._process_dir(self.gallery_dir, relabel=False)

        if verbose:
            print("=> Data loaded")
            self.print_dataset_statistics(train, query, gallery)#配置输出
    ...
    def get_imagedata_info(self, data):#拆分label、图片路径和摄像机ID分别获取
        pids, cams = [], []
    ...
    def _process_dir(self, dir_path, relabel=False):
        img_paths = glob.glob(osp.join(dir_path, '*.jpg'))#获得所有的jpg结尾的文件
        pattern = re.compile(r'([-\d]+)_c([\d]+)')#正则，搭配search

        pid_container = set()#创建一个无序不重复元素集
        for img_path in img_paths:
            pid, _ = map(int, pattern.search(img_path).groups())#将匹配得到的整体通过int函数
            if pid == -1: continue  # junk images are just ignored
            pid_container.add(pid)
        pid2label = {pid: label for label, pid in enumerate(pid_container)}#lable是人的标号？
