...
from evaluate import eval_func, euclidean_dist, re_rank_with_st
...
def test(args):
    ...
    distmat = dist_compute(query_feat, gallery_feat,
                           query_path, gallery_path, alpha=3)

    # result_distmat = {'distmat': distmat.numpy()}
    # io.savemat('./distmat.mat', result_distmat)

    cmc, mAP, all_AP = eval_func(distmat, query_pid.numpy(), gallery_pid.numpy(),
                                 query_camid.numpy(), gallery_camid.numpy(),
                                 use_cython=True)
    # cmc, mAP, all_AP = eval_func(distmat.numpy(), query_pid, gallery_pid,
    ...
    distmat = re_rank_with_st(query_feat, gallery_feat,
                              query_path, gallery_path, alpha=7)
