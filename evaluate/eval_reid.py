...
def eval_func(distmat, q_pids, g_pids, q_camids, g_camids, max_rank=50, use_cython=False):
    ...
    indices = np.argsort(distmat, axis=1) #axis=1表示每行排序，返回排序后下标[3,2,4]---[1,0,2]
    print(indices.shape)
    print(g_pids.shape, q_pids.shape)
    matches = (g_pids[indices] == q_pids[:, np.newaxis]).astype(np.int32)#q_pids[:, np.newaxis]，增加一个维度，将[1,2,3]变成[[1],[2],[3]]---(3,1)
